...
/**
 * @fileOverview This file defines a Genkit flow to refine a user's prompt for generating art using the Akash Chat API.
 *
...
export type SuggestBetterPromptInput = z.infer<
  typeof SuggestBetterPromptInputSchema
>;

const SuggestBetterPromptOutputSchema = z.string();
export type SuggestBetterPromptOutput = z.infer<
  typeof SuggestBetterPromptOutputSchema
>;
...
const systemPrompt = `You are an AI assistant for creative writing and prompt engineering for text-to-image models.
    ...
    try {
      const response = await akashClient.chat.completions.create({
        ...
        messages: [
          ...
          {
            role: 'user',
            content: `User's Prompt: "${input.prompt}"\n\nRefined Prompt:`,
          },
        ],
      });
